...
module RegisterAllocation = {
  ...
  let rec apply_allocations =
    ...
    | MSet(bind, i) =>
      MSet(
        apply_allocation_to_bind(bind),
        apply_allocations(allocs, i),
      )
    | MStore(bs) =>
      MStore(
        List.map(
...
let run_register_allocation = (instrs: list(Mashtree.instr)) => {
  ...
  let rec live_locals = (instr) =>
    ...
    | MSet(b, i) =>
      bind_live_local(b) @ live_locals(i)
    | MStore(bs) =>
      List.concat(
...
let rec compile_comp = (env, c) => {
  ...
  | [@implicit_arity] CAssign(arg1, arg2) =>
    // MSet(find_id(arg1, env), {instr_desc: MImmediate(compile_imm(env, arg2)), instr_loc: c.comp_loc})
    MBoxOp(MBoxUpdate(compile_imm(env, arg2)), compile_imm(env, arg1))
  ...
  | [@implicit_arity] CSetRecordItem(idx, record, arg) =>
    [@implicit_arity] MRecordOp(MRecordSet(idx, compile_imm(env, arg)), compile_imm(env, record))
